...
创建表单接收报价单号、精准 Odoo 地址、CSV 和 append/overwrite 模式。CSV 是简单的首列型号、次列正整数、可选第三列折扣百分比（空值表示不指定，不触碰 Odoo 折扣）格式，可选首行 header；不支持通用带引号/内嵌逗号 CSV 语法。报价号与 URL 至少提供一个，worker 优先尝试 URL，失败后按报价号搜索。
...
4. append 追加；overwrite 保留型号、数量（及指定折扣）均匹配的行，删除不在输入、数量不一致或指定折扣不一致的行后按输入重写；逐行上报成功/失败。CSV 未指定折扣的输入行不读取、不清零 Odoo 现有折扣。折扣必须晚于数量填写：Odoo 修改数量会清空已填折扣。
5. 尝试保存并读取最终 Odoo 行快照（含折扣）。随后把输入 CSV 与整表整体对比，逐条在日志反应多了/少了/不一致（带折扣行要求型号/数量/折扣一致，无折扣行只要求型号/数量一致；append 模式扣除确认阶段读到的预置行），不改变逐行结果与任务状态。
...
用户设置可用库存阈值、趋势警告阈值、近期月份和快速模式复选框（默认勾选；不勾选即全量检查，重读近期窗口，用于修复同步中断缺口）。`server/src/services/inventory.ts` 使用 SKU 清洗逻辑标准化 CSV，筛选 `freeToUse < threshold` 的型号，再让 worker 把 ATL/Stock 调动历史同步到本地库（`sku.sqlite` 的 `stock_moves`，增量或全量由截止时间矩阵决定），完成后从本地库聚合每个型号的近期出入库：
...
创建 job 后，页面只在查询期间订阅该 job 的 SSE，处理阶段（`moves-sync` 为调动同步阶段）、日志、低库存统计、逐项分类结果和终态。逐项 `trend-result` 事件在调动同步完成后一次性连续发出（事件形状不变，时序不再随逐项查询推进）；随后到达 `complete`。取消会向服务端请求中止 worker 任务，并在本地显示失败/用户取消。
...
- 自动库存下载和调动同步都依赖有效 Odoo profile；上传 CSV 只能绕过下载，不能绕过调动同步。
- 分类查询按产品名精确匹配本地库；Odoo 显示名与 CSV 清洗名不一致的型号会被聚合为 0 移动而落入"信息"，应结合日志判断。
- 报价 overwrite 的准确语义由 `auto/src/odoo/write-lines.ts` 决定；不要与 `script/quotation.ts` 的 DOM 覆写算法混为一谈。
- 折扣语义：CSV 第三列折扣为空 = 不指定（不读不清零 Odoo 折扣）；指定 0 表示显式要求无折扣，参与 overwrite 匹配。写入后不做逐行折扣回读，由最终整表校验统一在日志反映多了/少了/不一致。

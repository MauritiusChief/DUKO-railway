...
解析结果区始终可见（不再仅在解析后显示），空状态显示提示文字与"添加空行"按钮；有数据时该按钮位于底部操作行。
...
- **手动添加空行**：无需 AI 解析也可通过"添加空行"按钮建立空行，所有字段为空（`values: []`）、原始名称为空、数量为 1、状态为 `missing`。用户补全颜色/型号/尺寸后，失焦会触发 Exposed-Items 检查；未补全必填字段的行在生成产品时会被跳过的未解析行。添加空行会清空旧产品清单（与删除行行为一致）。

...
报价任务的详情与 SSE 会检查任务创建者或管理员权限。库存路由需要登录，但当前库存快照和 SSE 查询只按随机 job ID 查找，没有再次检查 job 所有者；这是现有安全边界，不应把 job ID 暴露给其他用户。

### Auto WebSocket

worker 主动连接 `/api/auto/connect`，以协议版本和 `AUTO_WORKER_TOKEN` 完成 `hello` 鉴权，然后发送 `ready`。当前只支持一个全局 worker 连接和单任务执行。
...
server 支持用户直接上传 CSV，跳过下载步骤，但趋势查验仍需要在线 worker。job、CSV 和分类结果只在内存中，服务重启后不可恢复。

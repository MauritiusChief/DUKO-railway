...
7. **iOS 解码后备**（2026-09-09 后续决定）：原「不引入第三方扫码 SDK」调整为原生 `BarcodeDetector` 优先、`barcode-detector` ZXing WASM ponyfill 后备，以支持 iOS 17+ Safari/Chrome。WASM 与前端构建产物同源部署，不依赖 CDN；保持拍照后识别，不增加持续相机预览。

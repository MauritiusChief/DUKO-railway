...
- 纬度范围为 `[-90, 90]`，经度范围为 `[-180, 180]`，`rangeKm` 必须为正数并设产品级最大值。
...
搜索端点一次用户请求最多放大为 3 次 Google 请求，因此不能只依赖通用 HTTP request limiter。除专用 limiter 外还要硬编码每次最多 3 页，并在 Google Cloud 设置项目配额和费用告警。

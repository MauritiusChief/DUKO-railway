...
1. 在 `server/src/middleware/rateLimit.ts` 新增 `warehouseDecodeLimiter`，每 IP `2400 次 / 15 分钟`。
2. 每秒两次持续 15 分钟为 `1800` 次；`2400` 为该基线保留约三分之一短时突发余量，显著高于 LLM 限流但独立于写入接口。
3. 解码吞吐由两个 worker、前端有界队列、图像缩放与硬超时共同控制；限流不替代容量保护。
4. 原 `warehouseScanLimiter` 暂保持不变，因为它限制的是用户确认后的数据库写入，不等同于每张图片的解码请求。若后续产品改为自动确认写入，必须同时重新评估该写入限额。
